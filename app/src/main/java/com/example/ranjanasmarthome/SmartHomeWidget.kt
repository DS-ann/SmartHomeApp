...
class SmartHomeWidget: AppWidgetProvider() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        when(action){
            ACTION_LIGHT1 -> toggleRelay(0)
            ACTION_LIGHT2 -> toggleRelay(4)
            ACTION_FAN1   -> toggleRelay(1)
            ACTION_FAN2   -> toggleRelay(5)
        }
    }

    private fun toggleRelay(index: Int) {
        val cmd = "$index${if(!toggleState["ACTION_$index"]!!) "1" else "0"}"
        toggleState["ACTION_$index"] = !toggleState["ACTION_$index"]!!
    ...
    private fun getPendingIntent(context: Context, action:String): PendingIntent {
        ...
        return PendingIntent.getBroadcast(context, action.hashCode(), intent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_MUTABLE)
    }
}
